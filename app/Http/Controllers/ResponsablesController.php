...
class ResponsablesController extends Controller
{
    public function index(Request $request)
    {
        ...
        $a = Unidadadmin::select('descrip')->where('unidad','=',trim($unidad->unidad))->first();
        $b = Unidadadmin::select('ciudad')->where('unidad','=',trim($unidad->unidad))->first();
            ...
            if($unidadv == ''){
                if ($buscar == ''){
                    $responsables = Responsables::join('unidadadmin','unidadadmin.unidad','=','resp.unidad')
                                ...
                                ->where('resp.unidad','=',trim($unidad->unidad))
                                ->distinct()
                                ->paginate(10);
                    return [
                        'pagination' => [
