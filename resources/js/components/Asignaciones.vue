...
export default {
  ...
  methods: {
    ...
    listarResponsable(page, buscar, criterio){
        let me=this;
        var url= '/responsable/buscar?page=' + page + '&buscar=' + buscar + '&criterio='+ criterio + '&unidad=' + this.idunidad;
        axios.get(url).then( (response) =>{
            var respuesta= response.data;
    ...
    registrarAsignacion1(){
        ...
        axios.put('/actual/actualizarasingacion',{
          ...
          window.open('http://192.168.140.102/actual/repAsignaciones?codofic=' + this.cod_ofi + '&codresp='+ this.cod_resp + '&unidad='+ this.idunidad +  '','_blank');
          this.cancelarAsignacion1();
        }).catch((error)=>{
            console.log(error);
        });
    ...
    registrarAsignacion2(){
        if (this.cod_ofi2==0 & this.cod_resp2==0) {
          swal.fire('Elija un Responsable','','error')
            ...
            Swal.fire(response.data.message, "", "success");
            window.open('http://192.168.140.102/actual/repAsignaciones?codofic=' + this.cod_ofi2 + '&codresp='+ this.cod_resp2 + '&unidad='+ this.idunidad +  '','_blank');
            this.cancelarAsignacion2();
            }).catch((error)=>{
                console.log(error);
            });
